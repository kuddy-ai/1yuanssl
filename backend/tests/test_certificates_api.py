...
@pytest.mark.asyncio
async def test_certificate_order_lifecycle(client: AsyncClient) -> None:
    ...
    challenges_response = await client.get(f"/api/v1/certificates/orders/{order_id}/challenges")
    ...
    validate_response = await client.post(f"/api/v1/certificates/orders/{order_id}/validate")

    assert validate_response.status_code == 200
    validated = validate_response.json()["data"]
    assert validated["status"] == "validating"

    issue_response = await client.post(f"/api/v1/certificates/orders/{order_id}/issue")
...
@pytest.mark.asyncio
async def test_dns01_order_returns_txt_record(client: AsyncClient) -> None:
    create_response = await client.post(
        "/api/v1/certificates/orders",
        json={
            "domains": ["*.example.com"],
